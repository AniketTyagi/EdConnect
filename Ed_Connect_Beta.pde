...
Classroom English = new Classroom("English", "Eidleman", MUSD);
Classroom History = new Classroom("History", "Hanks", MUSD);
...
void setup() {
  ...
  History.addAttribute("Memorization");
  ...
  double[] attributes1 = Math.students.get(MUSD.parseStudent("Tyagi", "Aniket")).attributeAverages.get(Math.students.get(MUSD.parseStudent("Tyagi", "Aniket")).attributeAverages.size() - 1);
  Math.addGrade("Tyagi", "Aniket", grades1, attributes1, month(), day(), year());
  
  double[] testWeights2 = { 0.5, 0.3, 0.1, 0.05, 0.05};
  double[] grades2 = {Math.gradePredict("Santos", "Christian", testWeights2), 1};
  double[] attributes2 = Math.students.get(MUSD.parseStudent("Santos", "Christian")).attributeAverages.get(Math.students.get(MUSD.parseStudent("Santos", "Christian")).attributeAverages.size() - 1);
  Math.addGrade("Santos", "Christian", grades2, attributes2, month(), day(), year());
...
void draw() {
  ...
  graphData(MUSD, classes.get(currentClass), lName, fName, 900, 500, 0, -120, 90);
  drawAttributeScan(960, 189, 230, 270, MUSD, classes.get(currentClass), lName, fName, attribute);
  drawAttributeRadar(960, 470, 230, 259, MUSD, classes.get(currentClass), lName, fName);
